...
############### Global Parameters ###############
# path
train_path = './data/detection_res_1218/training/'
test_path = './data/detection_res_1218/testing/'
demo_path = './data/detection_res_1218/testing/'
default_model_path = './demo_model/demo_model'
save_path = './models/predictor_accv16/'
...
class Model(nn.Module):
    ...
    def forward(self, X, y):
        ...
        for i in range(n_frames):
            ...
            pos_loss = -math.exp(-(n_frames-i-1)/20.0)* (-1 * self.loss(pred, y))
            # negative example
...
def test_all(model, path, device):
    ...
    for name, _, _, feat, ffeat, label in data_loader:
        ...
        if num_batch <=1:
            all_pred = pred[:,0:90]
            all_labels = np.reshape(label.cpu().numpy(),[N,1])
        else:
            all_pred = np.vstack((all_pred, pred[:,0:90]))
            all_labels = np.vstack((all_labels,np.reshape(label.cpu().numpy(),[N,1])))
    print(all_pred.shape, all_labels.shape)
    print(total_loss)
