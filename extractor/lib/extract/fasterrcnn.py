...
class Model(object):
  def __init__(self, args):
    ...
    if args.net == 'vgg16':
      ...
      fasterRCNN = resnet(cfg.classes, 101, pretrained=False, class_agnostic=args.class_agnostic)
    elif args.net == 'res50':
      fasterRCNN = resnet(cfg.classes, 50, pretrained=False, class_agnostic=args.class_agnostic)
    elif args.net == 'res152':
      fasterRCNN = resnet(cfg.classes, 152, pretrained=False, class_agnostic=args.class_agnostic)
    else:
  ...
  def run(self, ori_im, det_output, use_nms = True):
    ...
    if det_output:
      ...
      det_output.append_data(im2show)
    return res_feat.cpu().numpy(), res_dect.cpu().numpy()

class FasterRCNN(object):
    def __init__(self, dataset, det_vis):
        args = parse_args([])
        args.vis=det_vis
        if dataset=='DAD':
            args.checkepoch=6
            args.checkpoint=18197
            args.classes = np.asarray(['__background__',
                        'person', 'bike', 'motorbike', 'car', 'bus'])
        elif dataset=='EF':
            args.checkepoch=6
            args.checkpoint=39937
            args.classes = np.asarray(['__background__',
                        'object'])
